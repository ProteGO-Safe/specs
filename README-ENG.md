...
Since 13.03.2020, three teams: ProteGO (Bluetooth tracing) and SafeSafe (diagnosis and prevention), and Sigma Connectivity (Bluetooth tracing) were creating their solutions for mobile apps aimed to accelerate this process. The Ministry of Digital Affairs has merged these teams and established a new name for the app – ProteGO Safe. The app is prepared for the Chief Sanitary Inspectorate (further: GIS), which is also the controller of personal. The Minister of Digital Affairs and GovTech Poland, due to their experience, are supervising the development work.

The app is developed in compliance with principles arising from the General Data Protection Regulation (GDPR), including data minimization, privacy by design, privacy by default, accuracy, and confidentiality. Guidelines of the European Data Protection Board, European Commission, and the Toolbox developed under the eHealth Network. All parties involved pay particular attention to ensure the highest privacy standards. The whole project is entirely apolitical. Adopted solutions ensure the support for the health authorities in fighting the pandemic while using the minimal set of data necessary to accomplish that goal.
...
The “quality” of contact assessment is based on configuration (parameters) provided by the local Health Authority. These parameters determine how much the final results will be impacted by the duration of the meeting, the physical distance between the devices, and how much time has passed since the contact occurred. Based on these parameters, the analytical module provided by G + A gives the result corresponding to the risk of exposure to the virus.
...
If such contact has occurred, the application based on meeting time, the distance between devices, as well as other factors indicated in the GIS guidelines, will decide whether the end-user of the application should receive information about the potential threat of exposure.
...
After turning on the application, every user can check his personalized status:
...
The project is being developed at the request of the Ministry of Digital Affairs, by a consortium of companies: Tytani24 Sp. z o.o. (leaders), The Coders Sp. z o.o. , Webini Sp. z o.o. , Sigma Connectivity Sp. z o.o. , 25wat Sp. z o.o., Klimas Legal, Mobile Flag, HOLDAPP supported by all willing contributors.
...
Application is live since June 2020 with the full support of Exposure Notification API from Google and Apple. However, there is still a need for further development and maintenance of the application. Below is the list of planned changes, improvements, and new features:

- [4.3.X]: Support for other languages (EN, UA) with a possibility to add (propose) new translations by the community, Android app update to EN API 1.6, Google-based server update to follow changes required by EN API 1.6

- [4.4.X]: Possibility to report bug/issue in the application, Kill Switch feature (blocking app after the pandemic is over), UX/UI changes to improve the usability

- [4.5.X]: Interoperability - support for EU solution for exchanging TEKs with other countries through the [Federation Gateway solution](https://github.com/eu-federation-gateway-service), especially with DE [Corona Warn App](https://www.coronawarn.app/en/), Huawei no GMS phones support (using Core Contact Shield SDK)

## Interoperability

As most countries lower their restrictions and exit lock-down, there is an increase in people traveling to other countries. It is crucial to support these people with a solution that works not only in their home country but is compatible with applications introduced in other countries, too. Thanks to Exposure Notification API that is the most preferred technical solution for contact tracing in most EU countries, it is possible to make all applications that use it to co-exist and recognize contacts with positively diagnosed people from other countries. An eHealth Network group working under the European Commission is responsible for preparing a technical solution that will integrate all applications using Exposure Notification API. The proposed and currently discussed option is Federation Gateway that will be able to share user’s TEKs across the integrated countries. Discussions, first of all, take into account users’ data privacy aspects.

ProteGO Safe is involved in the eHealth Network group, plans to integrate with [Federation Gateway](https://github.com/eu-federation-gateway-service), and takes part in a piloting program for a couple of EU countries.
...
Guidelines and Code of Conduct for contributors are available here: [Contributing](.github/CONTRIBUTING.md). (Polish language version only)

## ProteGO Safe and it’s documentation is licensed under

The application’s source code is available under a well-known and open GPL-3.0 license. Link to the GitHub repository: https://github.com/ProteGO-Safe GNU General Public License v3.0 Permissions of this strong copyleft license are conditioned on making available complete source code of licensed works and modifications, which include larger works using a licensed work, under the same license. Copyright and license notices must be preserved. Contributors provide an express grant of patent rights.
